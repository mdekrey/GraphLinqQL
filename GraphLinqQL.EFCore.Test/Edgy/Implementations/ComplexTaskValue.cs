...
namespace GraphLinqQL.Edgy.Implementations
{
    internal class ComplexTaskValue : Interfaces.Complex.GraphQlContract<int>
    {
        ...
        public override IGraphQlScalarResult<int?> NullableInteger() =>
            this.Original().ResolveTask(value => Task.FromResult((int?)value));

        public override IGraphQlScalarResult<IEnumerable<int?>?> NullableIntegers() =>
            this.Original().ResolveTask(value => Task.FromResult(Enumerable.Repeat<int?>(value, 3).Concat(Enumerable.Repeat<int?>(null, 2))));

        public override IGraphQlScalarResult<string> Text() =>
            this.Original().ResolveTask(value => Task.FromResult(value.ToString()));

        public override IGraphQlScalarResult<IEnumerable<string>> Texts() =>
            this.Original().ResolveTask(value => Task.FromResult(Enumerable.Repeat(value.ToString(), 5)));

        public override IGraphQlScalarResult<string?> NullableText() =>
            this.Original().ResolveTask(value => Task.FromResult((string?)value.ToString()));
        ...
        public override IGraphQlObjectResult<Interfaces.Inner> Obj() =>
            this.Original().ResolveTask(value => Task.FromResult(value.ToString())).AsContract<Inner>();

        public override IGraphQlObjectResult<IEnumerable<Interfaces.Inner>> Objs() =>
            this.Original().ResolveTask(value => Task.FromResult(Enumerable.Repeat(value.ToString(), 5))).List(_ => _.AsContract<Inner>());
