...
namespace GraphQlResolver.StarWarsV3.Resolvers
{
    class Droid : Interfaces.Droid.GraphQlContract<Domain.Droid>
    {
        ...
        public override IGraphQlResult<IEnumerable<Character?>?> friends() =>
            Original.Resolve(droid => droid.Friends.Where(id => Domain.Data.humanLookup.ContainsKey(id)).Select(id => Domain.Data.humanLookup[id])).ConvertableList().As<Human>()
                .Union<IEnumerable<Character?>?>(Original.Resolve(droid => droid.Friends.Where(id => Domain.Data.droidLookup.ContainsKey(id)).Select(id => Domain.Data.droidLookup[id])).ConvertableList().As<Droid>());
