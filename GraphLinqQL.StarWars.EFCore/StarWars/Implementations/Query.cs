...
namespace GraphLinqQL.StarWars.Implementations
{
    public class Query : Interfaces.Query.GraphQlContract<GraphQlRoot>
    {
        ...
        public override IGraphQlObjectResult<Interfaces.Droid?> Droid(string id)
        {
            ...
            return this.ResolveTask(_ => FindDroidById(id)).Nullable(_ => _.AsContract<Droid>());
        }
        ...
        {
            var intId = int.Parse(id);
            return this.Resolve(dbContext.Humans.Where(human => human.Id == intId)).List(_ => _.AsContract<Human>()).Only();
        }

        public override IGraphQlObjectResult<IEnumerable<Interfaces.Review?>?> Reviews(Interfaces.Episode episode)
        {
            return this.Resolve(dbContext.Reviews.Where(review => review.Episode == InterfaceToDomain.ConvertEpisode(episode)))
        ...
        {
            var intId = int.Parse(id);
            return this.Resolve(dbContext.Starships.Where(starship => starship.Id == intId)).List(_ => _.AsContract<Starship>()).Only();
        }
    }
}
